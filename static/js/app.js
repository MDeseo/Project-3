...
function buildChart (sample, start_date, end_date) {

    var startdate = new Date(start_date.split('-').reverse().join('-'));
    // var startdate=toDate(start_date)
    // var enddate=toDate(end_date)
...
function initialize() {
    ...
    d3.csv(csvFile).then((data) => {
        ...
        let start_date = d3.select("#Start_date").property("value");
        let end_date = d3.select("#End_date").property("value");
        
        buildChart(bitcoinList[0], start_date, end_date);
...
// call the function to build the line chart
    buildChart(selDataset,start_date,end_date);
...
d3.selectAll("#filter-btn").on("click", optionChanged);
